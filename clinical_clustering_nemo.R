...
for(i in c("mri_results")){
  mritest=c(mritest,chisq.test(table(mri2$nemo,mri2[,i]))$p.value)
}
...
for(i in c("msq01_act_out_dreams","msq02_legs_jerk","msq03_restless_legs","msq04b_walked_asleep","msq05_snorted_awake","msq06_stop_breathing","msq07_leg_cramps","msq08.2")){
  sleeptest=c(sleeptest,chisq.test(table(sleep2$nemo,sleep2[,i]))$p.value)
}
sleep_aov=aov(sleep2$msq08_rate_of_alertness ~ sleep2$nemo, data = sleep2)
summary(sleep_aov)
leveneTest(sleep2$msq08_rate_of_alertness ~ as.factor(sleep2$nemo), data = sleep2)
plot(sleep_aov, 2)
shapiro.test(x = sleep_aov$residuals )
kruskal.test(sleep2$msq08_rate_of_alertness ~ sleep2$nemo, data = sleep2)
##################################################################################
...
for(i in colnames(mds_updrs12[,3939:3966])){
  mds_updrs1test=c(mds_updrs1test,chisq.test(table(mds_updrs12$nemo,mds_updrs12[,i]))$p.value)
}
corrplot(chisq.test(table(mds_updrs12$upd2108_pat_quest_daytime_sleepiness,mds_updrs12$nemo))$residuals, is.cor = FALSE,cl.align.text="l")
corrplot(chisq.test(table(mds_updrs12$upd2107_pat_quest_sleep_problems,mds_updrs12$nemo))$residuals, is.cor = FALSE,cl.align.text="l")
sum_score=(aov(mds_updrs12[,3969] ~ mds_updrs12$nemo, data = mds_updrs12))
leveneTest(mds_updrs12[,3969] ~ as.factor(mds_updrs12$nemo), data = mds_updrs12)
plot(sum_score, 2)
shapiro.test(x = sum_score$residuals )
kruskal.test(mds_updrs12[,3969] ~ mds_updrs12$nemo, data = mds_updrs12)
...
for(i in colnames(mds_updrs22[,3939:ncol(mds_updrs22)])){
  mds_updrs2test=c(mds_updrs2test,chisq.test(table(mds_updrs22$nemo,mds_updrs22[,i]))$p.value)
}
corrplot(chisq.test(table(mds_updrs22$upd2206_hygiene,mds_updrs22$nemo))$residuals, is.cor = FALSE)
corrplot(chisq.test(table(mds_updrs22$upd2209_turning_in_bed,mds_updrs22$nemo))$residuals, is.cor = FALSE)

summary(aov(mds_updrs22[,3965] ~ mds_updrs22$nemo, data = mds_updrs22))
kruskal.test(mds_updrs22[,3965] ~ mds_updrs22$nemo, data = mds_updrs22)
...
for(i in colnames(mds_updrs32[,3938:4009])){
  mds_updrs3test=c(mds_updrs3test,chisq.test(table(mds_updrs32$nemo,mds_updrs32[,i]))$p.value)
}
...
library(corrplot)
corrplot(chisq.test(table(mds_updrs32$nemo,mds_updrs32[,"code_upd2314_body_bradykinesia"]))$residuals, is.cor = FALSE)
corrplot(chisq.test(table(mds_updrs32$nemo,mds_updrs32[,"code_upd2317c_rest_tremor_amplitude_right_lower_extremity"]))$residuals, is.cor = FALSE)
corrplot(chisq.test(table(mds_updrs32$nemo,mds_updrs32[,"upd2314_body_bradykinesia"]))$residuals, is.cor = FALSE,cl.align.text="l")
corrplot(chisq.test(table(mds_updrs32$nemo,mds_updrs32[,"upd2317c_rest_tremor_amplitude_right_lower_extremity"]))$residuals, is.cor = FALSE,cl.align.text="l")
...
for(i in colnames(mds_updrs42[,c(3938:3950)])){
  mds_updrs4test=c(mds_updrs4test,chisq.test(table(mds_updrs42$nemo,mds_updrs42[,i]))$p.value)
}
# test assumptions
upd4_aov=aov(mds_updrs42[,3950] ~ mds_updrs42$nemo, data = mds_updrs42)
summary(upd4_aov)
leveneTest(mds_updrs42[,3950] ~ as.factor(mds_updrs42$nemo), data = mds_updrs42)
plot(upd4_aov, 2)
...
soma_plasma2=merge(x=genesprot2,y = soma_plasma,by = "row.names")
...
adl2=adl2[,-1]
# test assumptions
adl_aov=aov(adl2$mod_schwab_england_pct_adl_score ~ adl2$nemo, data = adl2)
summary(adl_aov)
leveneTest(adl2$mod_schwab_england_pct_adl_score ~ as.factor(adl2$nemo), data = adl2)
plot(adl_aov, 2)
...
for(i in colnames(pdq2[,3977:3984])){
  pdq_aov=aov(pdq2[,i] ~ pdq2$nemo, data = pdq2)
  pdqtest=c(pdqtest,summary(pdq_aov)[[1]]$`Pr(>F)`[1])
}

socialscore_aov=aov(pdq2$pdq39_social_score ~ pdq2$nemo, data = pdq2)
summary(socialscore_aov)
leveneTest(pdq2$pdq39_social_score ~ as.factor(pdq2$nemo), data = pdq2)
plot(socialscore_aov, 2)
shapiro.test(x = socialscore_aov$residuals )
kruskal.test(pdq2$pdq39_social_score ~ as.factor(pdq2$nemo), data = pdq2)
library("ggpubr")
ggboxplot(pdq2, x = "nemo", y = "pdq39_social_score", 
          color = "nemo",
          order = c("1", "2", "3","4"),
          ylab = "pdq39_social_score", xlab = "cluster")
pairwise.t.test(pdq2$pdq39_social_score, pdq2$nemo,
                p.adjust.method = "BH")
#"pdq39_20_angry","pdq39_36_felt_ignored"
# corrplot(chisq.test(table(pdq2$nemo,pdq2[,"pdq39_20_angry"]))$residuals, is.cor = FALSE)
# corrplot(chisq.test(table(pdq2$nemo,pdq2[,"pdq39_36_felt_ignored"]))$residuals, is.cor = FALSE)
...
for(i in c("datscan_visual_interpretation")){
  datscantest=c(datscantest,chisq.test(table(datscan2$nemo,datscan2[,i]))$p.value)
}
corrplot(chisq.test(table(datscan2$nemo,datscan2[,"datscan_visual_interpretation"]))$residuals, is.cor = FALSE)
...
# check asumptions
library(car)
Hserum_aov=aov(biospecimenH2$test_value ~ biospecimenH2$nemo, data = biospecimenH2)
summary(Hserum_aov)
leveneTest(biospecimenH2$test_value ~ as.factor(biospecimenH2$nemo), data = biospecimenH2)
plot(Hserum_aov, 2)
...
actinbeta_aov=aov(biospecimenAB2$test_value ~ biospecimenAB2$nemo, data = biospecimenAB2)
#################################################################################
...
glu_aov=aov(biospecimen_glu2$test_value ~ biospecimen_glu2$nemo, data = biospecimen_glu2)
summary(glu_aov)
leveneTest(biospecimen_glu2$test_value ~ as.factor(biospecimen_glu2$nemo), data = biospecimen_glu2)
plot(glu_aov, 2)
...
demo_aov=aov(demo2$age_at_baseline ~ demo2$nemo, data = demo2)
summary(demo_aov)
leveneTest(demo2$age_at_baseline ~ as.factor(demo2$nemo), data = demo2)
plot(demo_aov, 2)
shapiro.test(x = demo_aov$residuals )
kruskal.test(demo2$age_at_baseline ~ demo2$nemo, data = demo2)
demo_test=c()
for(i in c("sex","race","education_level_years","ethnicity")){
  demo_test=c(demo_test,chisq.test(table(demo2$nemo,demo2[,i]))$p.value)
}
################################################################################
...
for(i in colnames(mut2[,3935:3939])){
  mut_test=c(mut_test,chisq.test(table(mut2$nemo,mut2[,i]))$p.value)
}
corrplot(chisq.test(table(mut2$nemo,mut2[,"has_known_SNCA_mutation_in_WGS"]))$residuals, is.cor = FALSE)
...
for(i in colnames(mutape2[,3935:3936])){
  mutape_test=c(mutape_test,chisq.test(table(mutape2$nemo,mutape2[,i]))$p.value)
}
...
for(i in c("genetic_status_enrollment","genetic_status_wgs")){
  genstat_test=c(genstat_test,chisq.test(table(genstat2$nemo,genstat2[,i]))$p.value)
}
#################################################################################
...
abeta_aov=aov(abeta2$test_value ~ abeta2$nemo, data = abeta2)
summary(abeta_aov)
leveneTest(abeta2$test_value ~ as.factor(abeta2$nemo), data = abeta2)
plot(abeta_aov, 2)
shapiro.test(x = abeta_aov$residuals )
kruskal.test(abeta2$test_value ~ as.factor(abeta2$nemo), data = abeta2)
...
for(i in colnames(updr2[,3939:ncol(updr2)])){
  updrtest=c(updrtest,chisq.test(table(updr2$nemo,updr2[,i]))$p.value)
}
...
for(i in colnames(kolster2[,3939:ncol(kolster2)])){
  kolstest=c(kolstest,chisq.test(table(kolster2$nemo,kolster2[,i]))$p.value)
}
...
for(i in colnames(epw2[,3939:3954])){
  epwtest=c(epwtest,chisq.test(table(epw2$nemo,epw2[,i]))$p.value)
}
summary(aov(epw2[,3955] ~ epw2$nemo, data = epw2))
...
for(i in colnames(dti2[,3940:3947])){
  dti_aov=aov(dti2[,i] ~ dti2$nemo, data = dti2)
  dtitest=c(dtitest,summary(dti_aov)[[1]]$`Pr(>F)`[1])
}
for(i in colnames(dti2[,3938:3939])){
  print(chisq.test(table(dti2$nemo,dti2[,i]))$p.value)
}
corrplot(chisq.test(table(dti2$nemo,dti2[,3938]))$residuals, is.cor = FALSE)
corrplot(chisq.test(table(dti2$nemo,dti2[,3939]))$residuals, is.cor = FALSE)
...
for(i in colnames(upsit2[,3938:3943])){
  upsittest=c(upsittest,print(chisq.test(table(upsit2$nemo,upsit2[,i]))$p.value))
}
##################################################################################
...
for(i in c("moca_visuospatial_executive_subscore","moca_naming_subscore","moca_attention_digits_subscore","moca11_attention_vigilance","moca12_attention_serial_7s","moca13_sentence_repetition","moca15_verbal_fluency","moca_language_subscore","moca16_abstraction","moca_abstraction_subscore","moca_delayed_recall_subscore_optnl_cat_cue","moca_delayed_recall_subscore_optnl_mult_choice","moca_delayed_recall_subscore","moca_orientation_subscore","code_education_12years_complete")){
  mocatest=c(mocatest,(chisq.test(table(moca2$nemo,moca2[,i]))$p.value))
}
corrplot(chisq.test(table(moca2$nemo,moca2[,"moca_delayed_recall_subscore"]))$residuals, is.cor = FALSE)
...
for(i in c("tobacco_ever_used","tobacco_current_use","alcohol_ever_used","alcohol_current_use","tobacco_recent_use","tobacco_prior_use","tobacco_product_type","cigarettes_per_day","cigarettes_packs_per_day","alcohol_recent_use","alcohol_prior_use","alcohol_use_frequency","alcohol_drinks_daily_range","alcohol_six_more_drinks_frequency")){
  smoketest=c(smoketest,(chisq.test(table(smoke2$nemo,smoke2[,i]))$p.value))
}
corrplot(chisq.test(table(smoke2$nemo,smoke2[,"tobacco_recent_use"])[,-1])$residuals, is.cor = FALSE)
corrplot(chisq.test(table(smoke2$nemo,smoke2[,"tobacco_prior_use"])[,-1])$residuals, is.cor = FALSE)
summary(aov(smoke2[,"tobacco_start_age"] ~ smoke2$nemo, data = smoke2))
...
for(i in c("biological_mother_with_pd","biological_father_with_pd","other_relative_with_pd")){
  famtest=c(famtest,(chisq.test(table(family2$nemo,family2[,i]))$p.value))
}
...
med=med[med$visit_name=="M24",]
rownames(med)=med$participant_id
#rownames(genesprot2)=genesprot2$Row.names
...
for(i in c("on_levodopa","on_dopamine_agonist","on_other_pd_medications")){
  medtest=c(medtest,(chisq.test(table(med2$nemo,med2[,i]))$p.value))
}
corrplot(chisq.test(table(med2$nemo,med2[,"on_dopamine_agonist"]))$residuals, is.cor = FALSE)
################################################################################
